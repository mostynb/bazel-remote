...
type header struct {
	...
	// Offsets in the file on disk, of each compressed chunk, with an
	// additional entry for the end of the file.
	//
	// Stored as an int32 number of chunks, followed by their int64 offsets.
	// 4 bytes + (n+1)*8 bytes. With 1M chunks this imposes a 4.5PB upper
	// size limit on CAS blobs, which is fine for our purposes (we don't
	// expect to scale that nearly that high).
	chunkOffsets []int64
}

const chunkTableOffset = 8 + 1 + 4 + 4
...
func GetUncompressedReadCloser(f *os.File, expectedSize int64, offset int64) (io.ReadCloser, error) {
	...
	chunkNum := int32(offset / int64(h.chunkSize))
	...
	if chunkNum == int32(len(h.chunkOffsets)-2) {
